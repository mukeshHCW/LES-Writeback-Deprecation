...
## Detailed Steps to Enable the Writeback Feature
...
### Install Microsoft Entra Cloud Sync Provisioning Agent [Skip if you have Agent already installed]
...
### Configure Cloud Sync to Enable Exchange Attribute Writeback for Cloud-Managed Mailboxes

Exchange attribute writeback for cloud-managed mailboxes (Phase 2 of [Cloud-based management of Exchange attributes](https://learn.microsoft.com/en-us/exchange/hybrid-deployment/enable-exchange-attributes-cloud-management)) currently needs to be configured via Microsoft Graph API. A Cloud Sync UI experience will be available in a future release. The [full list of Exchange attributes that can be written back to On-Premises AD](https://learn.microsoft.com/en-us/exchange/hybrid-deployment/enable-exchange-attributes-cloud-management#identity-exchange-attributes-and-writeback) is available in the official documentation.
...
> |---------|-------------------------------|-------------------------------|
> | Configuration | Checkbox in Cloud Sync UI | MSGraph API (Cloudsync UI support to be released soon) |
...
#### Verify (and Enable) Organization for Sync
...
#### Create Service Principal for Exchange Attribute Writeback
...
#### Verify Job Creation and Get Job ID
...
#### Set Synchronization Job Secrets
...
**1. Get the user's Entra ID and the rule ID** (Assuming `$servicePrincipalId` and `$jobId` are already set from the Configure section above):
...
**2. Trigger on-demand provisioning:**
...
### Verify Exchange Attribute Writeback Configuration
...
> **Note:** This tests CustomAttribute1. The full list of 23 writeback attributes is documented in the **Attributes Written Back by Cloud Sync** section below.

2. Wait for sync cycle (delta sync runs every ~20 minutes)
